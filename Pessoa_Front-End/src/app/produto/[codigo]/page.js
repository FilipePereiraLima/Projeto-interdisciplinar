...
export default async function Produto({ params }) {
  ...
  const req = await fetch(
    'https://projeto-interdisciplinar-gamma.vercel.app/produto',
    {
      method: 'POST',
      cache: 'no-cache',
          ...
          <div className="hidden w-full md:block md:w-auto" id="navbar-default">
            <ul className="font-medium flex flex-col p-4 md:p-0 mt-4 border border-gray-100 rounded-lg  md:flex-row md:space-x-8 md:mt-0 md:border-0">
              <li>
                <a
                  href="/"
                  className="text-white block py-2 pl-3 pr-4 text-white bg-blue-700 rounded md:bg-transparent md:text-blue-700 md:p-0 dark:text-white md:dark:text-blue-500"
                >
                  Produtos
                </a>
              </li>
